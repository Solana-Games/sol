...
pub fn handler(ctx: Context<PlayGame>, bet_amount: u64, prediction: u8) -> Result<()> {
    ...
    // Validate prediction (1-6 for dice)
    require!(prediction >= 1 && prediction <= 6, CasinoError::InvalidPrediction);
    
    // Check vault has enough balance for potential payout
    let potential_payout = bet_amount * DICE_MULTIPLIER / 100;
    require!(
