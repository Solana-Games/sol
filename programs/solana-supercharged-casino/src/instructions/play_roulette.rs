...
pub fn handler(ctx: Context<PlayGame>, bet_amount: u64, prediction: u8) -> Result<()> {
    ...
    // Validate prediction (0-36 for roulette)
    require!(prediction <= 36, CasinoError::InvalidPrediction);
    
    // Check vault has enough balance for potential payout
    let potential_payout = bet_amount * ROULETTE_STRAIGHT_MULTIPLIER / 100;
    require!(
